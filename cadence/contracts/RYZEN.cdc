...
pub contract RYZEN: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        // Destroy the vault
        destroy() {
            RYZEN.totalSupply = RYZEN.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            RYZEN.totalSupply = RYZEN.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&RYZEN.Minter>(/public/Minter, target: /storage/MinterStorage)
